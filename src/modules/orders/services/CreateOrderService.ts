...
class CreateOrderService {
  ...
  public async execute({ customer_id, products }: IRequest): Promise<Order> {
    ...
    const existsProductsIds = existentsProducts.map(product => product.id);
    ...
    const findProductsWithNoQuantityAvaliable = products.filter(
      product => existentsProducts.filter(p => p.id === product.id)[0].quantity < product.quantity
    );

    if (findProductsWithNoQuantityAvaliable.length) {
      throw new AppError(`The quantity ${findProductsWithNoQuantityAvaliable[0].quantity} is not available for ${findProductsWithNoQuantityAvaliable[0].id}`);
    }
    ...
    const orderedProductsQuantity = order_products.map(product => ({
      ...
      quantity: existentsProducts.filter(p => p.id === product.product_id)[0].quantity - product.quantity
    }));
